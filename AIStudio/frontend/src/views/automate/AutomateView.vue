...
<el-button @click="adminDialogVisible = true">任务类型管理</el-button>
    ...
    <div class="list-toolbar">
      <span class="list-toolbar__title">执行记录</span>
      <el-select v-model="taskTypeFilter" placeholder="按任务类型筛选" clearable size="small" style="width: 200px">
        <el-option label="自定义任务" value="__custom__" />
        <el-option v-for="t in taskTypes" :key="t.code" :label="t.name" :value="t.code" />
      </el-select>
    </div>
    ...
    <el-table :data="filteredAutomates" stripe v-loading="loading">
      ...
      <el-table-column label="类型" width="110">
        <template #default="{ row }">
          <el-tag v-if="row.taskType" size="small" effect="plain">{{ taskTypeLabel(row.taskType) }}</el-tag>
      ...
      <el-table-column prop="status" label="状态" width="110">
        <template #default="{ row }">
          <el-tag :type="automateStatusType(row.status)" size="small">{{ automateStatusLabel(row.status) }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-empty v-if="!loading && automates.length === 0" description="暂无自动化任务" />
        ...
        <el-tabs v-model="detailActiveTab" @tab-change="handleDetailTabChange">
          ...
          <el-tab-pane label="基本信息" name="basic">
            <el-descriptions :column="2" border size="small">
              <el-descriptions-item label="TFS">#{{ selectedAutomate.tfsWorkItemId }}</el-descriptions-item>
              <el-descriptions-item label="标题">{{ selectedAutomate.tfsTitle }}</el-descriptions-item>
              <el-descriptions-item label="任务类型">{{ taskTypeLabel(selectedAutomate.taskType) }}</el-descriptions-item>
              <el-descriptions-item label="发起人">{{ selectedAutomate.createdBy || '-' }}</el-descriptions-item>
              <el-descriptions-item label="LLM 模型" :span="2">
                {{ selectedAutomate.model || '全局模型（环境变量配置）' }}
              </el-descriptions-item>
              <el-descriptions-item label="启动参数" :span="2">
                <span v-if="selectedAutomate.paramsJson" class="params-json">{{ selectedAutomate.paramsJson }}</span>
                <span v-else>-</span>
              </el-descriptions-item>
              <el-descriptions-item label="状态">
                <el-tag :type="automateStatusType(selectedAutomate.status)" size="small">{{ automateStatusLabel(selectedAutomate.status) }}</el-tag>
              </el-descriptions-item>
              <el-descriptions-item label="重试次数">{{ selectedAutomate.retryCount }}</el-descriptions-item>
              <el-descriptions-item label="Skill">
                {{ selectedAutomate.skillName || '-' }}
              </el-descriptions-item>
              <el-descriptions-item label="仓库 IDs">
                {{ selectedAutomate.repoIds || '-' }}
              </el-descriptions-item>
            </el-descriptions>
            <div v-if="selectedAutomate.error" class="automate-error">
              <h4>错误信息</h4>
              <pre>{{ selectedAutomate.error }}</pre>
            </div>
          </el-tab-pane>
          ...
          <el-tab-pane label="执行步骤" name="steps">
            ...
            <el-empty v-else-if="!selectedAutomate?.workflowDefinitionId" description="暂无工作流定义（Skill链模式）" />
            ...
            <div class="stages" v-if="selectedAutomate.stageResults && selectedAutomate.stageResults.length > 0">
              <h4>执行阶段</h4>
              <el-timeline>
                <el-timeline-item
          ...
          <el-tab-pane v-if="selectedAutomate.status === 'WAITING_CONFIRM'" label="确认" name="confirm">
            <div class="confirm-section" v-if="selectedAutomate.confirmMessage">
              <h4>确认消息</h4>
              <div class="confirm-message">
                <pre>{{ selectedAutomate.confirmMessage }}</pre>
              </div>
            </div>
            ...
            <div class="confirm-actions">
              <el-button type="success" @click="handleConfirm(selectedAutomate, true)" :loading="confirming">通过</el-button>
            ...
            <div v-if="showRejectFromDetail" class="reject-section">
              ...
              <el-button type="danger" size="small" @click="handleConfirm(selectedAutomate!, false)" :loading="confirming" style="margin-top: 8px">确认拒绝</el-button>
            </div>
          </el-tab-pane>
    ...
    <el-dialog v-model="showStepsDialog" title="执行步骤" width="900px">
      <div class="steps-toolbar">
        ...
        <span v-if="stepsAutomate" class="steps-automate-info">
          自动化任务 #{{ stepsAutomate.tfsWorkItemId }} - {{ stepsAutomate.tfsTitle }}
        </span>
      </div>
    ...
    <!-- Confirm Dialog -->
    <el-dialog v-model="showConfirmDialog" title="自动化任务确认" width="700px">
      <div v-if="confirmAutomate" class="confirm-dialog">
        <el-descriptions :column="2" border size="small" style="margin-bottom: 16px">
          <el-descriptions-item label="TFS">#{{ confirmAutomate.tfsWorkItemId }}</el-descriptions-item>
          <el-descriptions-item label="标题">{{ confirmAutomate.tfsTitle }}</el-descriptions-item>
          <el-descriptions-item label="Skill">{{ confirmAutomate.skillName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="状态">
        ...
        <div class="confirm-message-scroll">
          <pre>{{ confirmAutomate.confirmMessage || '无确认消息' }}</pre>
        </div>
        ...
        <div class="confirm-dialog-actions">
          <el-button type="success" size="large" @click="handleConfirm(confirmAutomate, true)" :loading="confirming">
            通过
          </el-button>
          ...
          <template v-else>
            <el-button type="danger" size="large" @click="handleConfirm(confirmAutomate, false)" :loading="confirming">
              确认拒绝
            </el-button>
    ...
    <el-dialog v-model="showContinueDialog" title="补充输入并继续" width="600px">
      <div v-if="continueAutomate" class="continue-dialog">
        <el-descriptions :column="2" border size="small" style="margin-bottom: 16px">
          <el-descriptions-item label="TFS">#{{ continueAutomate.tfsWorkItemId }}</el-descriptions-item>
          <el-descriptions-item label="标题">{{ continueAutomate.tfsTitle }}</el-descriptions-item>
    ...
    <el-dialog v-model="showChangesDialog" title="文件变更" width="80%">
      <div class="changes-toolbar">
        ...
        <span v-if="changesAutomate" class="changes-automate-info">
          自动化任务 #{{ changesAutomate.tfsWorkItemId }}
        </span>
    ...
    <el-dialog v-model="showArtifactsDialog" title="过程文件" width="900px">
      <div class="artifacts-toolbar">
        ...
        <span v-if="artifactsAutomate" class="artifacts-automate-info">
          自动化任务 #{{ artifactsAutomate.tfsWorkItemId }}
        </span>
      </div>
    ...
    <el-dialog v-model="showLogDialog" :title="logAutomate ? `自动化任务 #${logAutomate.tfsWorkItemId} - 日志` : '自动化任务日志'" width="900px">
      <div class="logs-toolbar">
        <el-button size="small" @click="refreshLogs" :loading="logsLoading">刷新</el-button>
        <span v-if="logAutomate" class="logs-automate-info">
          自动化任务 #{{ logAutomate.tfsWorkItemId }} - {{ logAutomate.tfsTitle }}
        </span>
    ...
    <TaskLaunchDialog
      ...
      :task-type="launchTaskType"
      @launched="handleTypedLaunched"
    />
...
import {
  ...
  type AutomateTask,
  type AutomateLog,
  type AutomateExecutionStep,
  type AutomateFileChange,
  type AutomateArtifact,
  type AutomateTaskType
} from '@/api/automate'
...
const launchTaskType = ref<AutomateTaskType | null>(null)
...
function taskTypeLabel(code?: string | null) {
  if (!code) return '自定义'
  return taskTypes.value.find(t => t.code === code)?.name || code
}

function handleLaunchType(type: AutomateTaskType | null) {
  if (type === null) {
    showStartDialog.value = true
  } else {
    launchTaskType.value = type
    launchDialogVisible.value = true
  }
}

function handleTypedLaunched(_task: AutomateTask) {
  loadAutomates()
}
...
const filteredAutomates = computed(() => {
  if (!taskTypeFilter.value) return automates.value
  if (taskTypeFilter.value === '__custom__') return automates.value.filter(a => !a.taskType)
  return automates.value.filter(a => a.taskType === taskTypeFilter.value)
})
...
function automateStatusType(status: string) {
...
function automateStatusLabel(status: string) {
  const map: Record<string, string> = {
    RUNNING: '运行中',
    SUCCESS: '成功',
    COMPLETED: '已完成',
...
const selectedAutomate = ref<AutomateTask | null>(null)
const detailSteps = ref<AutomateExecutionStep[]>([])
...
const detailChanges = ref<AutomateFileChange[]>([])
const detailArtifacts = ref<AutomateArtifact[]>([])
const detailChangesLoading = ref(false)
const detailArtifactsLoading = ref(false)
const detailLogs = ref<AutomateLog[]>([])
...
const continueAutomate = ref<AutomateTask | null>(null)
const continueNodeId = ref('')
const continueInput = ref('')
const continuing = ref(false)

function handleDetailTabChange(tab: string) {
  if (!selectedAutomate.value) return
  const id = selectedAutomate.value.id
  if (tab === 'changes' && detailChanges.value.length === 0) {
    detailChangesLoading.value = true
    automateApi.getChanges(id).then(data => { detailChanges.value = data }).catch(() => {}).finally(() => { detailChangesLoading.value = false })
  }
  if (tab === 'artifacts' && detailArtifacts.value.length === 0) {
    detailArtifactsLoading.value = true
    automateApi.getArtifacts(id).then(data => { detailArtifacts.value = data }).catch(() => {}).finally(() => { detailArtifactsLoading.value = false })
  }
  if (tab === 'logs' && detailLogs.value.length === 0) {
    detailLogsLoading.value = true
    automateApi.getLogs(id).then(data => { detailLogs.value = data }).catch(() => {}).finally(() => { detailLogsLoading.value = false })
  }
}
...
async function refreshDetailChanges() {
  ...
  detailChangesLoading.value = true
  try {
    detailChanges.value = await automateApi.getChanges(selectedAutomate.value.id)
  } catch {
...
async function refreshDetailArtifacts() {
  ...
  detailArtifactsLoading.value = true
  try {
    detailArtifacts.value = await automateApi.getArtifacts(selectedAutomate.value.id)
  } catch {
...
async function refreshDetailLogs() {
  ...
  detailLogsLoading.value = true
  try {
    detailLogs.value = await automateApi.getLogs(selectedAutomate.value.id)
  } catch {
...
async function loadWorkflowNodes(automate: AutomateTask) {
  if (!automate.workflowExecutionId) {
    workflowNodes.value = []
    return
  }
  try {
    const res = await http.get(`/workflows/executions/${automate.workflowExecutionId}/nodes`)
...
function isCurrentWorkflowNode(defNode: any) {
  ...
  if (!selectedAutomate.value?.workflowExecutionId && selectedAutomate.value) {
    const label = defNode.data?.label || defNode.id
    if (selectedAutomate.value.skillName === label) return true
  }
  return false
}
...
function canRetryWorkflowNode(node: any): boolean {
  if (!selectedAutomate.value?.workflowExecutionId) return false
  if (selectedAutomate.value.status === 'RUNNING') return false
...
function openContinueDialogFromNode(node: any) {
  if (!selectedAutomate.value) return
  continueAutomate.value = selectedAutomate.value
...
async function handleRetryWorkflowNode(node: any) {
  ...
  try {
    await automateApi.retryWorkflowNode(selectedAutomate.value.id, node.id)
    ElMessage.success('已重试节点，自动化任务将重新执行该节点及下游节点')
...
function openContinueDialog(row: AutomateTask) {
  continueAutomate.value = row
...
async function handleContinue() {
  if (!continueAutomate.value) return
  ...
  try {
    await automateApi.continueWorkflowNode(continueAutomate.value.id, continueNodeId.value, continueInput.value)
    ElMessage.success('已提交补充输入，继续执行')
    showContinueDialog.value = false
    await loadAutomates()
    if (showDetailDialog.value && selectedAutomate.value?.id === continueAutomate.value.id) {
      await refreshDetailDataSmart()
      startDetailAutoRefresh()
    }
  } catch {
...
async function loadWorkflowDefinition(automate: AutomateTask, forceReload = false) {
  // 如果不是强制重载，且已加载过，则只更新执行节点
  if (!forceReload && workflowDefinition.value && automate.workflowDefinitionId === workflowDefinition.value.id) {
    // 只更新执行节点（增量更新）
    if (automate.workflowExecutionId) {
      try {
        const res = await http.get(`/workflows/executions/${automate.workflowExecutionId}/nodes`)
  ...
  if (!automate.workflowDefinitionId) return
  try {
    const res = await http.get(`/workflows/${automate.workflowDefinitionId}`)
...
function getStageResult(stageKey: string) {
  if (!selectedAutomate.value?.stageResults) return null
  return selectedAutomate.value.stageResults.find(r => r.stage === stageKey)
}
...
function getStageTimelineType(stageKey: string) {
  ...
  if (!result) {
    ...
    const currentIdx = allStages.findIndex(s => s.key === selectedAutomate.value?.currentStage)
...
async function showDetail(automate: AutomateTask) {
  try {
    selectedAutomate.value = await automateApi.get(automate.id)
    ...
    try {
      detailSteps.value = await automateApi.getSteps(automate.id)
    } catch {
      detailSteps.value = []
    }
    // 加载工作流节点（兼容旧逻辑）
    await loadWorkflowNodes(selectedAutomate.value)
    // 加载工作流定义（用于"执行步骤" Tab 显示定义节点）
    await loadWorkflowDefinition(selectedAutomate.value)
    // 主动加载过程文件和文件变更
    try {
      detailChanges.value = await automateApi.getChanges(automate.id)
    } catch {
      detailChanges.value = []
    }
    try {
      detailArtifacts.value = await automateApi.getArtifacts(automate.id)
    } catch {
      detailArtifacts.value = []
    }
    // Start auto-refresh if automate is active
    startDetailAutoRefresh()
  } catch {
...
function startDetailAutoRefresh() {
  ...
  // 启用自动刷新，但只在数据变化时更新界面
  if (selectedAutomate.value &&
      (selectedAutomate.value.status === 'RUNNING' || selectedAutomate.value.status === 'WAITING_CONFIRM')) {
...
async function refreshDetailDataSmart() {
  ...
  const id = selectedAutomate.value.id
  try {
    const newData = await automateApi.get(id)

    // 只在状态变化时更新基本信息，避免不必要的重渲染
    let automateChanged = false
    if (selectedAutomate.value.status !== newData.status ||
        selectedAutomate.value.currentStage !== newData.currentStage ||
        selectedAutomate.value.retryCount !== newData.retryCount) {
      selectedAutomate.value = newData
      automateChanged = true
    }

    // 只刷新当前活动的 tab
    if (detailActiveTab.value === 'steps') {
      const newSteps = await automateApi.getSteps(id)
      // 只在 automate 状态变化时重新加载工作流定义节点，避免频繁刷新
      if (automateChanged) {
        await loadWorkflowDefinition(selectedAutomate.value)
      }
      ...
      const newChanges = await automateApi.getChanges(id)
      ...
      const newArtifacts = await automateApi.getArtifacts(id)
      ...
      const newLogs = await automateApi.getLogs(id)
...
const stepsAutomate = ref<AutomateTask | null>(null)
const stepsList = ref<AutomateExecutionStep[]>([])
...
async function openSteps(automate: AutomateTask) {
  stepsAutomate.value = automate
  showStepsDialog.value = true
  await refreshSteps()
  startStepsAutoRefresh(automate)
}

async function refreshSteps() {
  if (!stepsAutomate.value) return
  stepsLoading.value = true
  try {
    stepsList.value = await automateApi.getSteps(stepsAutomate.value.id)
  } catch {
...
function startStepsAutoRefresh(automate: AutomateTask) {
  stopStepsAutoRefresh()
  if (automate.status === 'RUNNING' || automate.status === 'WAITING_CONFIRM') {
...
const confirmAutomate = ref<AutomateTask | null>(null)
const showRejectInput = ref(false)

async function openConfirm(automate: AutomateTask) {
  try {
    const fresh = await automateApi.get(automate.id)
    confirmAutomate.value = fresh
    showRejectInput.value = false
    rejectComment.value = ''
    showConfirmDialog.value = true
  } catch {
    ElMessage.error('获取自动化任务信息失败')
  }
}

async function handleConfirm(automate: AutomateTask, approved: boolean) {
  confirming.value = true
  try {
    await automateApi.confirm(automate.id, approved, rejectComment.value || undefined)
    ...
    if (showDetailDialog.value && selectedAutomate.value?.id === automate.id) {
      selectedAutomate.value = await automateApi.get(automate.id)
    }
  } catch {
...
const changesAutomate = ref<AutomateTask | null>(null)
const changesList = ref<AutomateFileChange[]>([])
...
async function openChanges(automate: AutomateTask) {
  changesAutomate.value = automate
  showChangesDialog.value = true
  await refreshChanges()
}

async function refreshChanges() {
  if (!changesAutomate.value) return
  changesLoading.value = true
  try {
    changesList.value = await automateApi.getChanges(changesAutomate.value.id)
  } catch {
    ElMessage.error('加载文件变更失败')
  } finally {
...
const artifactsAutomate = ref<AutomateTask | null>(null)
const artifactsList = ref<AutomateArtifact[]>([])
...
async function openArtifacts(automate: AutomateTask) {
  artifactsAutomate.value = automate
  showArtifactsDialog.value = true
  await refreshArtifacts()
}

async function refreshArtifacts() {
  if (!artifactsAutomate.value) return
  artifactsLoading.value = true
  try {
    artifactsList.value = await automateApi.getArtifacts(artifactsAutomate.value.id)
  } catch {
    ElMessage.error('加载过程文件失败')
  } finally {
...
const logs = ref<AutomateLog[]>([])
const logsLoading = ref(false)
const logAutomate = ref<AutomateTask | null>(null)
let logAutomateId: number | null = null
let logTimer: ReturnType<typeof setInterval> | null = null

async function openLogs(automate: AutomateTask) {
  logAutomateId = automate.id
  logAutomate.value = automate
...
async function refreshLogs() {
  if (logAutomateId == null) return
  logsLoading.value = true
  try {
    logs.value = await automateApi.getLogs(logAutomateId)
  } catch {
    // ignore
  } finally {
...
async function loadAutomates() {
  loading.value = true
  try {
    automates.value = await automateApi.list()
  } catch {
    ElMessage.error('加载自动化任务列表失败')
  } finally {
    loading.value = false
  }
}

async function loadAutomatesData() {
  try {
    return await automateApi.list()
  } catch {
    return automates.value
  }
}
...
async function handleRetry(automate: AutomateTask) {
  try {
    await automateApi.retry(automate.id)
    ElMessage.success('自动化任务已重试')
    await loadAutomates()
  } catch {
...
function startTableAutoRefresh() {
  ...
  tableTimer = setInterval(async () => {
    ...
    const needsRefresh = automates.value.some(
      p => p.status === 'RUNNING' || p.status === 'WAITING_CONFIRM'
    )
    if (needsRefresh) {
      const newAutomates = await loadAutomatesData()
      // 只在数据真正变化时更新
      if (JSON.stringify(automates.value) !== JSON.stringify(newAutomates)) {
        automates.value = newAutomates
      }
    }
  }, 10000)
}
