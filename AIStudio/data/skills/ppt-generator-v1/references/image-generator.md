...
|------------|-------------------|--------------|
| Prompt document | `project/images/image_prompts.md` | **Must** be saved using file write tool â€?cannot just be output in conversation |
...
```
Primary: #1E3A5F (Deep Navy)  â†? "deep navy blue (#1E3A5F)"
Secondary: #F8F9FA (Light Gray) â†? "light gray (#F8F9FA)"
Accent: #D4AF37 (Gold)        â†? "gold accent (#D4AF37)"
...
When generating multiple images for a single deck, visual coherence is critical. Use a **Deck Style Anchor** â€?a shared prefix of 15-25 words prepended to every image prompt.
...
1. Full-page / large-area backdrop â†?**Background** (3.1)
2. Real scenes / people / products â†?**Photography** (3.2)
3. Flat / illustration / cartoon style â†?**Illustration** (3.3)
4. Process / architecture / relationships â†?**Diagram** (3.4)
5. Partial decoration / texture â†?**Decorative Pattern** (3.5)
...
**Identifying characteristics**: Real scenes, people, products, architecture â€?photographic quality
...
1. **Determine type** â†?Background / Photography / Illustration / Diagram / Decorative
2. **Understand purpose** â†?Which page? What function?
3. **Analyze original description** â†?Information from the user's "Generation description"
4. **Apply type-specific key points** â†?Reference the corresponding type's table
5. **Generate optimized prompt** â†?Use the 2.1 standard output format
6. **Save prompt document** â†?**Must** write to `project/images/image_prompts.md`
...
|---------|------|-----------|
| **Default** â€?no explicit override from the user | **Path A**: `image_gen.py` CLI | Uses `.env` `IMAGE_BACKEND` configuration |
...
#### Path A â€?`image_gen.py` CLI (Default)
...
#### Path B â€?Host-Native Image Tool (On Explicit User Request)
...
- Executor downstream is path-agnostic â€?no spec change required between Path A and Path B

#### Failure Handling (Applies to Both Paths)

If generation fails for a given image:

1. **Retry once.** If the retry also fails, stop attempting that image â€?do not loop further.
