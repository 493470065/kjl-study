...
**Core principle (side-by-side)**: compute container layout from the image's original aspect ratio so the image displays completely â€?no excess whitespace, no cropping.

> **Scope**: this spec applies to *side-by-side* intent only. Other intents (hero / full-bleed, atmosphere / background, accent / inline) use full-bleed placement where ratio alignment is not a constraint and cropping is expected â€?the ratioâ†’split table below does NOT apply. See `references/strategist.md` Â§h for intent selection.
...
```
1. Decide narrative intent (hero / atmosphere / side-by-side / accent) â€?see strategist.md Â§h
2. If intent = side-by-side: continue below. Otherwise: compose per narrative; this spec does not apply.
3. Get image original dimensions â†?Calculate ratio (width/height)
...
> Boundary ratio (e.g., 1.5): decide by text volume â€?more text â†?left-right; less text â†?top-bottom.
...
If not satisfied â†?Switch to left-right layout
```
...
```
Original: 1960x800, R=2.45 â†?Top-bottom split
Image: 1160x473, Text area: 1160x147 â†?7:3 top-bottom
```

### Standard Landscape (ratio 1.38)

```
Original: 1614x1171, R=1.38 â†?Left-right split
Image: 773x560 (left), Text area: 367x560 (right) â†?7:3 left-right
```
...
Try top-bottom: image height=663, text area=-43 â?Switch to left-right: image 780x446 (left), text area 360x600 (right) â†?7:3 left-right
```
...
Default selection table assumes **landscape or square canvas**. For portrait canvases (height > width), left-right splits leave both columns too narrow â€?use the override below.
...
Complete display (data charts, side-by-side â€?must not crop):
...
`--canvas` selects target format (default `ppt169`). The tool computes layout type (top-bottom / left-right), image display area, and text area per the formulas above. Output is a Markdown table â€?paste directly into the image resource list.
