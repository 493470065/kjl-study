# Executor Consultant â€?Consulting Style
...
> Px values assume the typical dense consulting baseline (`body` â‰?18px). For a deck with a different `body` in `spec_lock.md`, apply the ratios in parentheses rather than copying the px figures.
...
- **Direct data labels**: Place values at the top of bar charts â€?no legend needed
...
Main trunk on the left, branches expanding rightward â€?mutually exclusive, collectively exhaustive:

```
Main metric â”€â”€â”¬â”€â”€ Branch A (45%)
              â”œâ”€â”€ Branch B (30%)
              â”œâ”€â”€ Branch C (20%)
              â””â”€â”€ Branch D (5%)
```
...
Top-level metric â†?decomposed factors â†?actions. Ideal for performance attribution analysis:

```
Revenue growth 15%
  â”œâ”€â”€ Average order value +8%  â†?Premium product line expansion
  â”œâ”€â”€ Customer count +5%       â†?New channel acquisition
  â””â”€â”€ Repurchase rate +2%      â†?Loyalty program optimization
```
...
> When `page_rhythm = breathing`, this standard layout can legitimately degenerate to **chart-dominant (2:8)**: one large chart carrying the page, one sentence of takeaway â€?the rhythm rule is then satisfied without adding patterns foreign to consulting style. See `executor-base.md Â§2.1` for the universal rhythm discipline.
...
Consultant-style speaker notes use a **conclusion-driven** approach â€?state the conclusion first, then present supporting evidence. Professional, concise, and persuasive tone.
...
| `[Data]` | Cue to verbalize numbers conversationally | "[Data] 85% â†?more than eight out of ten customers reported this issue" |
...
- **Conversational data**: 30% â†?"roughly one-third", 85% â†?"more than eight out of ten", 2.5x â†?"two and a half times"
...
- [ ] Data matches source data â€?no fabrication
