...
description: >
  ...
  "生成PPT", "做PPT", "制作演示文稿", or mentions "win-ppt".
---
...
>
> - `win-ppt` is a repository-specific workflow, not a general application scaffold
...
### Step 3: Template Selection (BLOCKING)

🚧 **GATE**: Step 2 complete; project directory structure is ready.

**🔒 BLOCKING**: This step requires user interaction. The AI MUST present available template options and wait for explicit user selection before proceeding.

**Available Templates**: Read `${SKILL_DIR}/templates/layouts/layouts_index.json` to get the list of available templates.

**Presentation Format**: Present the template options in a clear, structured format with descriptions:

| Template Name | Description |
|---------------|-------------|
| 卫宁健康-AI汇报 | AI汇报风格，适合科技、数据相关内容 |
| 卫宁健康-橘色 | 橘色主题，活力热情的视觉风格 |
| 卫宁健康-深蓝色 | 深蓝色主题，专业稳重的视觉风格 |
| 卫宁健康-暖调大地色 | 暖调大地色主题，自然舒适的视觉风格 |

**User Prompt**: Ask the user to select a template:

> 请问您希望使用哪个PPT模板？
>
> 1. 卫宁健康-AI汇报 - AI汇报风格，适合科技、数据相关内容
> 2. 卫宁健康-橘色 - 橘色主题，活力热情的视觉风格
> 3. 卫宁健康-深蓝色 - 深蓝色主题，专业稳重的视觉风格
> 4. 卫宁健康-暖调大地色 - 暖调大地色主题，自然舒适的视觉风格

**Template Copy**: Once user selects a template, copy the template files:

```bash
cp ${SKILL_DIR}/templates/layouts/<selected_template>/*.svg <project_path>/templates/
cp ${SKILL_DIR}/templates/layouts/<selected_template>/design_spec.md <project_path>/templates/
cp ${SKILL_DIR}/templates/layouts/<selected_template>/*.png <project_path>/images/ 2>/dev/null || true
cp ${SKILL_DIR}/templates/layouts/<selected_template>/*.jpg <project_path>/images/ 2>/dev/null || true
```

> To create a new global template, read `workflows/create-template.md`

**�?Checkpoint �?Template files copied, proceed to Step 4.**
