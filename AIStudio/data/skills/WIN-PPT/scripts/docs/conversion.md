...
- `.docx` â€?via `mammoth`
- `.html` / `.htm` â€?via `markdownify` + `beautifulsoup4`
- `.epub` â€?via `ebooklib` + `markdownify`
- `.ipynb` â€?via `nbconvert`
...
```bash
# Native path â€?always required
pip install mammoth markdownify ebooklib nbconvert beautifulsoup4

# Fallback path â€?only for .doc/.odt/.rtf/.tex/.rst/.org/.typ
...
For most users the Python version is sufficient â€?Node.js is no longer
