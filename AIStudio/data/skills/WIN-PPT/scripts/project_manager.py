#!/usr/bin/env python3
"""win-ppt project management helpers.
    ...
    def import_sources(
        ...
        for item in source_items:
            ...
            if copy:
                ...
                effective_move = True
                print(
                    f"note: {source_path} is inside the win-ppt repo; moved "
...
def list_templates() -> None:
    ...
    import json
    templates_dir = SKILL_DIR / "templates" / "layouts"
    index_path = templates_dir / "layouts_index.json"
    
    if not index_path.exists():
        print("[ERROR] Template index not found:", index_path)
        return
...
def import_template(project_path: str, template_name: str) -> None:
    """Import a template into the project."""
    templates_dir = SKILL_DIR / "templates" / "layouts"
    ...
    template_files = list(template_dir.glob("*.svg")) + list(template_dir.glob("*.md"))
    image_files = list(template_dir.glob("*.png")) + list(template_dir.glob("*.jpg"))
    
    templates_output = project_path / "templates"
    templates_output.mkdir(parents=True, exist_ok=True)
    
    images_output = project_path / "images"
    images_output.mkdir(parents=True, exist_ok=True)
    
    for file in template_files:
        dest = templates_output / file.name
        shutil.copy(str(file), str(dest))
    
    for file in image_files:
        dest = images_output / file.name
        shutil.copy(str(file), str(dest))
    
    print(f"[OK] Template '{template_name}' imported successfully!")
    print(f"  - SVG templates: {len(template_files)} files")
    print(f"  - Image assets: {len(image_files)} files")
...
def main() -> None:
    ...
    try:
        ...
        if command == "import-template":
            if len(sys.argv) < 4:
                raise ValueError("Usage: project_manager.py import-template <project_path> <template_name>")
            project_path = sys.argv[2]
            template_name = sys.argv[3]
            import_template(project_path, template_name)
            return
