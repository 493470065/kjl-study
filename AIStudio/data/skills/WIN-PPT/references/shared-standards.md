...
The following are **forbidden** in generated SVGs ‚Ä?PPT export breaks otherwise:
...
|---|---|---|
| Typography & symbols (em dash, en dash, ¬©, ¬Æ, ‚Ü? ¬∑, NBSP, full-width punctuation, emoji‚Ä? | **Raw Unicode characters** ‚Ä?write `‚Äî` `‚Äì` `¬©` `¬Æ` `‚Üí` directly | HTML named entities ‚Ä?`&mdash;` `&ndash;` `&copy;` `&reg;` `&rarr;` `&middot;` `&nbsp;` `&hellip;` `&bull;` etc. |
| XML reserved characters (`&`, `<`, `>`, `"`, `'`) | **XML entities only** ‚Ä?`&amp;` `&lt;` `&gt;` `&quot;` `&apos;` (e.g. `R&amp;D`, `error &lt; 5%`) | Bare `&` `<` `>` (e.g. `R&D`, `error < 5%`) |
...
> **`marker-start` / `marker-end` is conditionally allowed** ‚Ä?see ¬ß1.1 for constraints. The converter maps qualifying markers to native DrawingML `<a:headEnd>` / `<a:tailEnd>`.
>
> **`clipPath` on `<image>` is conditionally allowed** ‚Ä?see ¬ß1.2 for constraints. The converter maps qualifying clip shapes to native DrawingML picture geometry (`<a:prstGeom>` or `<a:custGeom>`).
>
> **Replacing `<mask>` effects** ‚Ä?DrawingML has no per-pixel alpha. Route by effect:
> - Image gradient overlay (vignette/fade/tint) ‚Ü?stacked `<rect>` with `<linearGradient>`/`<radialGradient>` (¬ß6 Image Overlay)
> - Non-rectangular image crop (circle/rounded/hexagon) ‚Ü?`clipPath` on `<image>` (¬ß1.2)
> - Inner glow / soft-edge ‚Ü?`<filter>` with `<feGaussianBlur>` (¬ß6 Glow)
> - Drop shadow ‚Ü?filter shadow or layered rect (¬ß6 Shadow)
>
> Pixel-level alpha effects (text-knockout image fills, arbitrary alpha composites) have no PPT path ‚Ä?bake into the source image at Image_Generator stage.
...
| Marker child's `fill` **matches** the parent line's `stroke` color | In DrawingML the arrow head inherits the line color ‚Ä?a mismatched marker fill will look wrong on export. |
| `markerWidth` / `markerHeight` roughly in `3‚Ä?5` range | Mapped to `sm` (<6) / `med` (6‚Ä?2) / `lg` (>12) size buckets. |
...
**Recommended template** ‚Ä?a standard arrow-head definition ready to reuse:
...
> ‚ö†Ô∏è Unclassifiable marker shapes (curved paths, multi-segment, >4 vertices) are silently dropped ‚Ä?line renders without arrow. Use a manual `<polygon>` for exotic shapes.
...
- NOT on shapes (`<rect>`/`<circle>`/`<path>`/`<g>`/`<text>`) ‚Ä?draw the target shape directly. A rect clipped to a circle is just a circle.
...
**Recommended template** ‚Ä?circular image clip:
...
**Rounded rectangle clip** ‚Ä?for card-style image frames:
...
> ‚ö†Ô∏è `clip-path` on non-image elements is FORBIDDEN ‚Ä?quality checker errors out. Draw target geometry directly.
...
> Arrows: prefer `marker-end` for connector lines (¬ß1.1) ‚Ä?converter produces native auto-rotating arrow heads. For block/chunky arrows, use standalone closed shapes; see `templates/charts/chevron_process.svg` and `templates/charts/process_flow.svg`.
...
- **Fonts**: every `font-family` stack MUST end with a pre-installed family (Microsoft YaHei / SimSun / Arial / Times New Roman / Consolas ‚Ä?; `@font-face` is FORBIDDEN. Full rule: [`strategist.md ¬ßg`](strategist.md).
...
One logical line ‚Ä?even with mixed colors/weights/sizes ‚Ä?MUST be one `<text>` with inline `<tspan>` children. Never use multiple adjacent `<text>` elements. The converter maps each `<tspan>` to a `<a:r>` run within the same PPT text frame, keeping the line as one editable shape.

‚ú?**DO** ‚Ä?one `<text>` ‚Ü?one text frame with three runs:

```xml
<text x="100" y="200" font-size="24" fill="#333333">
  ÂÆûÁé∞<tspan fill="#1A73E8" font-weight="bold">10ÂÄ?/tspan>ÊïàÁéáÊèêÂçá
</text>
```

‚ù?**DON'T** ‚Ä?three side-by-side `<text>` elements become three separate text frames in PPT (breaks edit-as-one-line, risks alignment drift, makes spacing fragile):

```xml
<text x="100" y="200" font-size="24" fill="#333333">ÂÆûÁé∞</text>
<text x="160" y="200" font-size="24" fill="#1A73E8" font-weight="bold">10ÂÄ?/text>
<text x="240" y="200" font-size="24" fill="#333333">ÊïàÁéáÊèêÂçá</text>
```

**‚ö†Ô∏è Inline tspans must NOT carry `x`/`y`/`dy`** ‚Ä?those mark a new line, and `flatten_tspan` will split into a separate text frame. `dx` is safe (kerning, stays inline). Only set `x`/`y`/`dy` on tspans that genuinely start a new line.

**Multi-line `<text>` with per-line emphasis works**: an outer line-break tspan (with `x` + `dy` or `y`) MAY contain nested inline tspans for color/weight/size ‚Ä?converter walks nested tspans and emits one run per styled segment:

```xml
<text x="80" y="190" font-size="18" fill="#333333">
  <tspan x="80" dy="0">ÂÆåÊàêÁé?tspan fill="#4CAF50" font-weight="bold">98%</tspan>Ë∂ÖÈ¢ÑÊú?/tspan>
  <tspan x="80" dy="35">ÊàêÊú¨Èôç‰Ωé<tspan fill="#F44336" font-weight="bold">¬•120‰∏?/tspan></tspan>
</text>
```

‚ù?**DON'T** ‚Ä?same-line column jump via `<tspan x="...">`:

```xml
<text x="100" y="200" font-size="18" fill="#333333">
  <tspan x="100">Â∑¶Âàó</tspan><tspan x="600" font-weight="bold">Âè≥Âàó</tspan>
</text>
```

`x` on a tspan starts a new line, splitting into two independent text frames. For two-column layouts, write two `<text>` elements.

**Default ‚Ä?lift key information.** Uniform-styled paragraphs read as walls of text. Wrap these in `<tspan fill="..." font-weight="bold">`:

- **Numerical results** ‚Ä?percentages, multipliers (`10x`), absolute amounts (`¬•120‰∏á`)
- **Contrasts** ‚Ä?gain/loss, before/after, target/actual
- **One or two load-bearing nouns per sentence** ‚Ä?the term that carries the insight
...
‚ù?**DON'T** ‚Ä?uniform-styled paragraph buries the insight:

```xml
<text x="80" y="200" font-size="20" fill="#333333">
  2024Âπ¥ÂÖ¨Âè∏Ëê•Êî∂ÂêåÊØîÂ¢ûÈï?5%ËææÂà∞12‰∫øÂÖÉÂàõÂéÜÂè≤Êñ∞È´?</text>
```

‚ú?**DO** ‚Ä?same line, key data lifted:

```xml
<text x="80" y="200" font-size="20" fill="#333333">
  2024Âπ¥ÂÖ¨Âè∏Ëê•Êî∂ÂêåÊØ?tspan fill="#1A73E8" font-weight="bold">Â¢ûÈïø35%</tspan>ËææÂà∞<tspan fill="#1A73E8" font-weight="bold">12‰∫øÂÖÉ</tspan>ÂàõÂéÜÂè≤Êñ∞È´?</text>
```
...
> ‚ö†Ô∏è Only `<g opacity="...">` is banned (¬ß2). Plain `<g>` for grouping is required.

**Animation-ready rule**: direct children of `<svg>` should be semantic groups, not raw drawing atoms. Aim for **3‚Ä? top-level `<g id>` groups per slide**; each group should correspond to one presenter click if page-level animation is enabled.

**What to group**:

| Grouping Unit | Contains |
|---------------|----------|
| Card / panel | Background rect + (optional shadow only if the card floats over a photo/colored panel ‚Ä?see ¬ß6) + icon + title + body text |
...
<g id="card-benefits-1">
  <!-- This card floats over a colored panel ‚Ä?shadow is appropriate. On a flat white canvas, omit the filter. -->
  ...
  <text x="105" y="270" font-size="56" font-weight="bold" fill="#0071E3">10√ó</text>
...
Must be executed in order ‚Ä?skipping or adding extra flags is FORBIDDEN:
...
# Output:
#   exports/<project_name>_<timestamp>.pptx           ‚Ü?main native pptx
#   backup/<timestamp>/<project_name>_svg.pptx        ‚Ü?SVG snapshot
#   backup/<timestamp>/svg_output/                    ‚Ü?Executor SVG source backup
```

**Prohibited**:
- NEVER use `cp` as a substitute for `finalize_svg.py`
- NEVER export directly from `svg_output/` ‚Ä?MUST export from `svg_final/` (use `-s final`)
...
> `<mask>` elements and `<image opacity="...">` are banned. Always use stacked `<rect>` or gradient overlays instead (see ¬ß2).
...
- Background panels / dividers / decorative bars ‚Ä?they are the floor
- Equal peer cards in a 2/3/4-up grid ‚Ä?keep all flat
- Containers with visible border, gradient fill, or strong tint ‚Ä?redundant
- Body-text paragraph containers ‚Ä?disrupts scan rhythm
- Decorative lines / dividers / icons ‚Ä?they are symbols, not objects
- Pages with only one content container ‚Ä?no second layer to lift above
- Dark backgrounds ‚Ä?black shadows vanish; use 1px low-opacity white stroke or outer glow

**Per-page budget**: ‚â?-3 shadowed elements. If you reach for a 4th, drop one first.
...
|------|------|----|--------------|---------------|
| Floor (no shadow) | Backgrounds, peer-grid cards, dividers, body-text containers | ‚Ä?| ‚Ä?| ‚Ä?|
...
#### Filter Soft Shadow ‚Ä?Recommended
...
```
stdDeviation:   4‚Ä?6       (resting cards: 4‚Ä?;  raised elements: 10‚Ä?6)
flood-opacity:  0.06‚Ä?.12  (resting cards ‚Ä?default)
                0.12‚Ä?.20  (raised elements only ‚Ä?primary CTA, overlay)
                NEVER     > 0.20  (Office 2007 hard-shadow look)
dy:             2‚Ä?0       (resting: 2‚Ä?;  raised: 6‚Ä?0)
dx:             0‚Ä?        (must match every other shadow on the page ‚Ä?single light source)
```
...
Replace `flood-color` with the element's brand color. Keep `flood-opacity` 0.12-0.20. Reserve for the single primary CTA per page ‚Ä?using on every button defeats the cue.
...
```
stdDeviation:   4‚Ä?      (smaller = subtle, larger = prominent)
flood-color:    brand color or accent color (NOT black)
flood-opacity:  0.35‚Ä?.55  (stronger than shadow for visibility)
```

**vs shadow**: no `<feOffset>` (or dx=0/dy=0). The converter uses this to distinguish glow from shadow.

#### Layered Rect Shadow ‚Ä?High-Compatibility Fallback

Best for: maximum compatibility with older PowerPoint versions. Stack 2‚Ä? semi-transparent rectangles behind the main card:
...
#### Linear Gradient Overlay ‚Ä?Most Common

Best for: image+text pages. Gradient direction should match text position (text on left ‚Ü?gradient darkens toward left).
...
#### Radial Gradient Overlay ‚Ä?Vignette Effect
...
|----------|-----------------------|-------|
| Card / panel shadow (only when floating over photo/colored panel) | Filter soft shadow (`flood-opacity` 0.06‚Ä?.12, single light source) | Hard black shadow, full-page abundance |
...
| Accent / CTA button (one per page) | Colored shadow (same hue family, `flood-opacity` 0.12‚Ä?.20) | Generic gray shadow, applying to every button |
...
### stroke-dasharray ‚Ä?Dashed / Dotted Lines
...
### Gradient Fill ‚Ä?linearGradient & radialGradient
...
**Linear gradient** ‚Ä?best for buttons, header bars, background panels:
...
**Radial gradient** ‚Ä?best for spotlight backgrounds, circular accents:
...
### transform: rotate ‚Ä?Element Rotation
...
### Arc Paths ‚Ä?Donut / Pie Charts

Calculate arc endpoint coordinates precisely with trigonometry. Never estimate ‚Ä?small errors produce wildly wrong shapes.

**Calculation formula** (center `cx,cy`, radius `r`, angle `Œ∏` in degrees):
```
x = cx + r √ó cos(Œ∏ √ó œÄ / 180)
y = cy + r √ó sin(Œ∏ √ó œÄ / 180)
```

**Key rules**:
1. Start at **-90¬∞** (12 o'clock position) and go clockwise
2. Each sector spans `percentage √ó 360¬∞`
...
**Example ‚Ä?75% donut sector** (center 400,400, outer r=180, inner r=100):
```
Start angle: -90¬∞    ‚Ü?outer(400, 220), inner(400, 300)
End angle: -90+270=180¬∞ ‚Ü?outer(220, 400), inner(300, 400)
...
> For connector lines prefer `marker-end`/`marker-start` (¬ß1.1). For chunky/wide solid/non-connector arrows, use standalone polygon or path.
...
**Method** ‚Ä?calculate triangle points using the line's direction vector:

```
Given line from (x1,y1) to (x2,y2):
1. Direction vector: dx = x2-x1, dy = y2-y1
2. Normalize: len = ‚à?dx¬≤+dy¬≤), ux = dx/len, uy = dy/len
3. Perpendicular: px = -uy, py = ux
4. Arrow tip = (x2, y2)
5. Back point 1 = (x2 - ux√ó12 + px√ó5,  y2 - uy√ó12 + py√ó5)
6. Back point 2 = (x2 - ux√ó12 - px√ó5,  y2 - uy√ó12 - py√ó5)
```

**Example ‚Ä?diagonal line** from (260,310) to (370,430):
```
dx=110, dy=120, len‚â?62.8, ux=0.676, uy=0.737
px=-0.737, py=0.676
Tip: (370, 430)
...
‚ö†Ô∏è Never use a fixed downward/rightward triangle on a diagonal line ‚Ä?arrow will point wrong.
...
```
project/
‚îú‚îÄ‚îÄ svg_output/    # Raw SVGs (Executor output, contains placeholders)
‚îú‚îÄ‚îÄ svg_final/     # Post-processed final SVGs (finalize_svg.py output)
‚îú‚îÄ‚îÄ images/        # Image assets (user-provided + AI-generated)
‚îú‚îÄ‚îÄ notes/         # Speaker notes (.md files matching SVG names)
‚î?  ‚îî‚îÄ‚îÄ total.md   # Complete speaker notes document (before splitting)
‚îú‚îÄ‚îÄ templates/     # Project templates (if any)
‚îî‚îÄ‚îÄ *.pptx         # Exported PPT file
```
