# Executor General â€?Creative Versatile Style
...
- **Visual weight balance**: Dark/large-area elements are "heavy", light/small elements are "light" â€?balance left-right/top-bottom
...
| Rounded rectangle cards | Content containers, feature modules | `rx="12"`; add light shadow ONLY if the card floats over a photo/colored panel (see shared-standards.md Â§6) |
...
Sizes follow the ramp anchored on the deck's `body` baseline from `spec_lock.md` â€?the px values below are **example figures for body â‰?18px** (multiply by the ratio column for any other baseline: 0.7x annotation / 1x body / 1.2x subtitle / 1.6x title).

```
Title layer   â†?~1.5-2x body  (e.g., 28-36px @ body=18)   bold, theme color or white
Subtitle layer â†?~1.2x body   (e.g., 20-24px @ body=18)   medium weight, secondary color
Body layer    â†?1x body       (e.g., 16-18px @ body=18)   regular, dark gray
Annotation layer â†?~0.7-0.8x  (e.g., 12-14px @ body=18)   light gray, bottom-aligned
```
...
General style speaker notes use **conversational narration** â€?like talking with the audience, not reading a report. Natural tone with rhythm, using rhetorical devices where appropriate.
...
- **Conversational data**: 30% â†?"nearly one-third", 2.5x â†?"more than doubled"
...
Fast â€?deployment time cut from 3 months to 2 weeks; [Pause]
Accurate â€?recognition accuracy at 97.3%, far exceeding the industry average of 82%;
Efficient â€?overall costs reduced by nearly one-third.
